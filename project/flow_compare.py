...
def compare_js():
    ...
    dat = (datetime.datetime.now()-datetime.timedelta(days=2)).strftime("%Y-%m-%d")
    ...
    for point in p_nms:
        ...
        flow_compare = pd.DataFrame(np.zeros((len(flow_forecast3),19)))
        ...
        flow_compares.append(flow_compare)
        
    flow_compares = pd.concat(flow_compares)
    flow_compares = flow_compares.replace(np.inf,np.nan).replace(np.nan,None)
